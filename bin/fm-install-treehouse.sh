...
# treehouse (spawn worktree acquisition), and by the operator install path:
# bin/fm-bootstrap.sh's install_cmd offers this script rather than upstream's
# `curl | sh`, so an operator install gets the same integrity guarantee CI does.
# Same pin/checksum discipline as fm-install-herdr.sh: official release URL,
# exact asset, SHA-256, bounded download, post-install version check. Never a
