...
| `fm-fleet-feed.sh`       | Project the fleet snapshot, registry and clone reads onto the dashboard's fleet feed contract |
| `fm-dashboard-refresh.sh` | Manual trigger that regenerates the fleet feed, rebuilds the dashboard page in its own checkout outside `projects/`, and publishes it |
...
| `fm-project-write-pretool-check.sh` | Primary-home PreToolUse guard refusing a file write under `projects/` (hard rule 1) |
...
| `fm-wake-lib.sh`         | Shared durable wake queue, recovery generations, portable locks, canonical path resolution and comparison, and watcher identity/health helpers |
...
| `fm-pr-merge.sh`         | Record the stated merge authority and PR metadata, then merge a task's canonical full GitHub URL |
