...
A crew that declares `paused:` for a known external wait is separately absorbed while idle and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge, with a live agent as much as a dead one.
The re-surface throttle survives transient resets such as idle pane churn and momentary busy blips, so the recheck stays once per pause window and retires only with the pause itself.
For a durable `captain-held` endpoint, or a declared pause whose authoritative state reader has fallen back to stopped or unknown, the normal-mode watcher applies that cadence only when the backend confidently reports its agent dead; a live or inconclusively read agent there first surfaces one stale wake, and the secondmate idle-endpoint exemption is unchanged.
...
The feed also folds each registered secondmate's durable work from the snapshot's bounded mate summaries into the owning project's card, merged by default with the mate named in supporting text, counting each item once on its own state, and discloses both a mate it could not fully read and work it read but could not place on any project; the script header owns that fold contract.
...
A GitHub pull request URL and a GitLab merge request URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)) both go through that one guarded path: every validation, metadata, and approval-record step is shared, and only the execution step forks to `gh-axi pr merge` or `glab mr merge`.
The helper defaults to `--squash`, preserves an explicit merge-method flag, and rejects malformed URLs or project override flags before recording merge state.
It also refuses to merge at all without `--authority captain|yolo`, accepts `--authority yolo` only when the task's own meta records `yolo=on`, and writes `state/<id>.merge-approval` before calling the forge, so hard rule 2 has runtime backing rather than prose alone.
On GitLab it additionally pins the reviewed head with `glab mr merge --sha`, merges immediately and non-interactively rather than deferring to auto-merge, and reads a pipeline as green only when that pipeline ran on the exact head being merged; [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns that contract.
