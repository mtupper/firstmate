...
It also refuses to merge at all without `--authority captain|yolo`, accepts `--authority yolo` only when the task's own meta records `yolo=on`, and writes `state/<id>.merge-approval` before calling `gh-axi`, so hard rule 2 has runtime backing rather than prose alone; [`bin/fm-pr-merge.sh`](../bin/fm-pr-merge.sh)'s header owns that contract.
