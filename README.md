...
- **A fleet feed you can render** - `bin/fm-fleet-feed.sh` turns this home's project registry, backlog, task records and status logs into the fleet feed a dashboard renders, emitting only what it can ground and refusing to write at all when a source is missing or malformed.
