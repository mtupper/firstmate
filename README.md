...
- **A fleet feed and a dashboard you can refresh** - `bin/fm-fleet-feed.sh` turns this home's project registry, backlog, task records and status logs into the fleet feed a dashboard renders, omitting every field this home holds no evidence for instead of inventing one, and refusing the whole run - leaving the previous feed intact - rather than writing a half feed when the evidence it cannot skip is missing or malformed. One manual command, `bin/fm-dashboard-refresh.sh`, refreshes that feed, rebuilds the standalone dashboard page from it in a build checkout outside your clones, and publishes it where the page is served, refusing a second run while one is live and leaving the last published page in place whenever a run fails.
