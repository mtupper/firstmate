...
Pull requests targeting `main` are reviewed on what CI says about them.
Every PR must pass the checks in `.github/workflows/ci.yml`: lint, the test-coverage guard, the behavior suites, stock-macOS Bash compatibility, and the repo invariants.
No pipeline is mandatory to open one.

## Workflow

1. Fork the repo, then clone the parent repo or set your local `origin` back to the parent (`git@github.com:kunchenguid/firstmate.git`).
2. Create a branch and make your changes.
3. Commit your changes, push the branch to your fork, and open a PR against the parent repo.
4. Watch CI and fix what it reports.

## Optional: raising a PR through no-mistakes

[`no-mistakes`](https://github.com/kunchenguid/no-mistakes) remains fully supported and is worth reaching for on a change you want reviewed hard before it is seen.
It puts a local git proxy in front of your real remote: pushing through it runs an AI-driven review/test/lint pipeline in an isolated worktree, forwards the push upstream only after every check passes, and opens a clean PR automatically.

1. Initialize it with your fork as the push target: `no-mistakes init --fork-url git@github.com:<you>/firstmate.git` (firstmate expects **no-mistakes v1.31.2+**; without a fork, plain `no-mistakes init` still works for maintainers with push access).
2. Push through the gate instead of pushing to `origin`:

   ```sh
   git push no-mistakes
   ```

3. Run `no-mistakes` to attach to the pipeline, watch findings, authorize auto-fixes, and review ask-user findings as needed.
   Follow the installed no-mistakes version's SKILL.md and live `axi` help for gate mechanics.
4. Once the pipeline passes, it pushes the branch to your fork and opens the PR against the parent repo for you.
...
- A path written into a durable record is canonicalized first, through `fm_canonical_path` in [`bin/fm-wake-lib.sh`](bin/fm-wake-lib.sh).
  A directory reachable through a symlink has two valid spellings, and a record keyed on the alias is invisible to the other spelling.
  `fm_canonical_path` takes either shape: it resolves a directory with `cd <dir> && pwd -P`, and it resolves anything else - a file, or a path that does not exist yet - by resolving the parent and keeping the final component.
  Where that lib is not sourced, write the same two forms by hand rather than a second idiom.
  Comparing a stored path against a live one uses `fm_same_path`, which resolves both sides so records written before canonicalization still match.
